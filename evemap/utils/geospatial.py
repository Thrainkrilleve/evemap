...
class Geospatial:
    ...
    def regions():
        ...
        for conn in connections:
            id1, id2 = conn
            if id1 not in stargates or id2 not in stargates:
                continue
            sg1 = stargates[id1]
            sg2 = stargates[id2]
            feature = {
    ...
    def region_stargates_lines():
        ...
        for gate in EveStargate.objects.all():
            ...
            if desto_gate_id is None:
                replacement_gate = EveStargate.objects.filter(
                    eve_solar_system=gate.destination_eve_solar_system
                ).first()
                desto_gate_id = replacement_gate.id if replacement_gate else None
